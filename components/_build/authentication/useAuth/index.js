...
export const AuthProvider = ({
  ...
  const signIn = async ({
    ...
    const signIn = await import("../functions/".concat(service, "/auth.").concat(search));
    const {
      error: signInError,
      user: signedInUser,
  ...
  const checkIfSignedIn = async () => {
    const authCheck = await import("../functions/".concat(service, "/auth.check"));
    const {
      user: userRes
    } = await authCheck.default();
    ...
    if (userRes) {
      ...
      const {
        user: cmUser
      } = await cmModule.default();
      if (cmUser) setUser(cmUser);else setUser(false);
    }
  };

  const signOut = async () => {
    const signOutFunc = await import("../functions/".concat(service, "/auth.signOut"));
