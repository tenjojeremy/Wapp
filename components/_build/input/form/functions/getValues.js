...
const serialize = function (form) {
  ...
  let formValues = {}; // Loop through each field in the form

  for (let i = 0; i < form.elements.length; i++) {
    let selections = [];
    let field = form.elements[i]; // console.log(field)
    // console.log(field.type)
    // Don't serialize fields without a name, submits, buttons, file and reset inputs, and disabled fields

    if (!field.name || field.disabled || field.type === 'reset' || field.type === 'submit' || field.type === 'button') continue; // handle text, number, checkbox, radio fields

    if (field.type === 'text' || field.type === 'number' || field.type !== 'checkbox' && field.type !== 'radio' || field.checked) {
