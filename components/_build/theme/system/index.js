...
export default (props => {
  ...
  const minWidthMediaQueries = removeDuplicatesInArray({
    data: mediaQueries.minWidth
  });
  const minHeightMediaQueries = removeDuplicatesInArray({
    data: mediaQueries.minHeight
  });

  const handleVariableValue = ({
    varName,
    value
  }) => {
    ...
    const withVariable = "var(--".concat(varName, "-").concat(cleanValue, ")");
  ...
  const getString = ({
    ...
    const _value = handleVariableValue({
      varName,
      value
    });

    if (!_key || !_value) return '';
    return isArray ? handleMediaQueries({
      key,
      value,
      varName
  ...
  const handleMediaQueries = ({
    ...
    const getMediaQueryValue = (_key, item, index, type) => {
      ...
      const property = "".concat(_key, ": ").concat(_value, ";");
      return index === 0 ? property : _value ? "@media (min-".concat(type, ": ").concat(queries[index], "px) {\n          ").concat(property, "\n        }") : '';
    }; // handle min WIDTH
  ...
  const handleStyles = ({
    ...
    Object.entries(styles).forEach(([key, value]) => {
      ...
      } else if (key.includes('padding') || key.includes('margin') || key.includes('gap') || key.includes('Gap') || key.includes('top') || key.includes('right') || key.includes('bottom') || key.includes('left')) array.push(getString({
        varName: 'spacing',
        key,
        value
      }));else if (key.includes('color') || key.includes('Color') || key.includes('fill')) array.push(getString({
        varName: 'color',
        key,
        value
      }));else array.push(getString({
        key,
        value
      }));
    });
  ...
  const handleSelectors = () => {
    ...
    if (selectors.length) {
      const se = selectors.map(({
        selector,
        styles
      }) => {
        let string = "".concat(selector, " {");
        string += handleStyles({
          styles
        });
        string += "}";
  ...
  const genStyles = handleStyles({
    styles
  });
  const selectorStyles = handleSelectors({
    selectors
  });
  return "".concat(genStyles, " ").concat(selectorStyles);
});