...
export const UploadsProvider = ({
  ...
  const [hasFetchedAll, sethasFetchedAll] = useState(null); // Functions
  ...
  const uploadsCreate = async ({
    data,
    location
  }) => {
    setCreating(true);
    setUploads((await _uploadsCreate({
      state: uploads,
      data,
      location
    })));
  ...
  const uploadsUpdate = async ({
    id,
    toUpdate
  }) => {
    setUpdating(true);
    setUploads((await _uploadsUpdate({
      state: uploads,
      id,
      toUpdate
    })));
  ...
  const uploadsDelete = async ({
    id
  }) => {
    setDeleting(true);
    setUploads((await _uploadsDelete({
      state: uploads,
      id
    })));
    setDeleting(false);
  ...
  return /*#__PURE__*/React.createElement(UploadsContext.Provider, {
    value: {
      uploads,
      uploadsCreate,
      uploadsRead,
