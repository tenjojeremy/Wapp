...
export default (async (_userId, _collection, _payload) => {
  ...
  if (userId && collection && payload) {
    ...
    throw new Error("Parameters with false need a value ".concat((userId, collection, payload)));
  }
});
...
const getById = async (userId, collection, payload) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      docRef = await db.collection(collection).doc(payload).get();
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (payload.sortBy && payload.where) {
        docsRef = await db.collection(collection).where(payload.where.field, payload.where.condition, payload.where.value).orderBy(payload.sortBy.field, payload.sortBy.type).get();
      } else if (payload.where) {
        docsRef = await db.collection(collection).where(payload.where.field, payload.where.condition, payload.where.value).get();
      }
