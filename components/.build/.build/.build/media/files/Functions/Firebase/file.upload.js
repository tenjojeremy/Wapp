...
export default (async (userId = null, files = null, collection = null) => {
  ...
  if (files && userId && collection) {
    ...
    fileNameThumb = ''.concat(compressedFile.name + currentDate, '_thumb'); // fullsize image

    fullsizeImg_file = await imagesResize(compressedFile, 800);
    fullsizeImg_savePath = 'users/'.concat(userId, '/').concat(fileName);
    fullsizeImg_fileUrl = await storageAdd(userId, fullsizeImg_savePath, fullsizeImg_file); // thumb image

    thumbImg_file = await imagesResize(compressedFile, 300);
    thumbImg_savePath = 'users/'.concat(userId, '/').concat(fileName, '_thumb');
    ...
    throw new Error('Parameters with false need a value '.concat((files, userId, collection)));
  }
});