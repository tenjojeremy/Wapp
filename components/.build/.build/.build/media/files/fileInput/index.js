...
const FileInput = ({
  accept,
  children = /*#__PURE__*/React.createElement('button', null, 'Upload'),
  onInput,
  name,
  validation,
  returnBase64,
  ...
  const errMessagesRef = useRef(null); // Tracks validilty with react

  const [isValid, setIsValid] = useState(validation.length === 0); // Tracks validilty with data attribute for use with Form component
  ...
  const checkValidation = async file => {
    const messages = await validation.map(async ({
      ...
      const mod = await import('./validation/'.concat(name, '/fileInput.').concat(name, '.js'));
  ...
  const handleFileUpload = async () => {
    ...
    await Promise.all(Array.from(files).map(async file => {
      ...
      if (currentErrorMessages.length > 0) {
        ...
        const fileBase64 = returnBase64 ? await fileReader({
          file,
          fullCompress: true
        }) : undefined;
        filesList.push({
  ...
  return /*#__PURE__*/React.createElement(Wrapper, null, /*#__PURE__*/React.createElement(Input, _extends({
    ...
    name: name
  }, nativeProps)), /*#__PURE__*/React.createElement(ChildrenWrap, null, children), /*#__PURE__*/React.createElement(Animation, {
    name: 'showHide',
    show: !isValid,
    el: errMessagesRef
  ...
  }, errorMessages.map(message => /*#__PURE__*/React.createElement(Typography, {
    key: message,
    text: message,
    color: 'red',
    variant: 'body2'
