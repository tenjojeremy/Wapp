...
let fileString = '{ '
const packageName = '@tenjojeremy/wapp'

glob(`components/_build/**/index.js`, function(err, files) {
  if (err) throw err
...
"${longName}": {
  ...
  fs.writeFile('tenjo-wapp-snippets/snippets/snippets.json', fileString, function(err) {
    if (err) throw err
    console.log(`${successEmogi}  snippet.json created`)
  })

  // 3. Add snippets to README
  fs.writeFile('tenjo-wapp-snippets/README.md', fileString, function(err) {
    if (err) throw err
